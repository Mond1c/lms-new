...
const IdentityHttpTimeout = 10 * time.Second
...
func New(ctx context.Context, cfg *config.Config) (*App, error) {
	...
	a.pool = pool
	a.shutdownFns = append(a.shutdownFns, func(context.Context) error {
		pool.Close()
		return nil
	})
...
func (a *App) shutdown() error {
	a.ready.Store(false)
	ctx, cancel := context.WithTimeout(context.Background(), IdentityHttpTimeout)
	defer cancel()
